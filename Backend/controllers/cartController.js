...
exports.getCart = async (req, res) => {
  try {
    // Lấy user_id từ query, header, body hoặc token
    const user_id = req.query.user_id || req.headers['x-user-id'] || req.body.user_id || (req.user ? req.user._id : null);

    console.log('getCart - user_id sources:');
    console.log('- query:', req.query.user_id);
    console.log('- header:', req.headers['x-user-id']);
    console.log('- body:', req.body.user_id);
    console.log('- token user:', req.user ? req.user._id : null);
    console.log('- final user_id:', user_id);

    if (!user_id) {
      return res.status(400).json({
        success: false,
        message: 'Vui lòng cung cấp user_id trong query (?user_id=xxx), header (x-user-id), hoặc body'
      });
    }
...
exports.updateCartItem = async (req, res) => {
  try {
    const { product_id, id } = req.params; // Support both product_id and id
    const { user_id, quantity } = req.body;

    console.log('updateCartItem - params:', req.params);
    ...
    // If id is provided (item_id), find cart item directly
    if (id) {
    ...
    const itemIndex = cart.items.findIndex(
      item => item.product_id.toString() === targetId
    );
