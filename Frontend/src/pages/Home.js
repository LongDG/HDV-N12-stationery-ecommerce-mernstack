...
const Home = () => {
  ...
  const [allProducts, setAllProducts] = useState([]); // Thêm state lưu tất cả sản phẩm
    ...
    const fetchData = async () => {
      try {
        const [categoriesRes, productsRes, allProductsRes] = await Promise.all([
          ...
          productService.getProducts({ limit: 1000 }) // Lấy tất cả sản phẩm cho việc đếm
        ]);
        ...
        if (allProductsRes.success) {
          setAllProducts(allProductsRes.data?.products || allProductsRes.data || []);
        }
      } catch (error) {
  ...
  const getCategoryIcon = (categoryName) => {
    ...
    // Mapping danh mục với icon phù hợp
    if (name.includes('bút') || name.includes('viết')) {
      return '✏️'; // bút viết
    }
    if (name.includes('giấy') || name.includes('tờ') || name.includes('in')) {
      return '📄'; // giấy tờ
    }
    if (name.includes('sổ') || name.includes('tập') || name.includes('note')) {
      return '📖'; // sổ tập
    }
    if (name.includes('bìa') || name.includes('file') || name.includes('folder')) {
      return '📁'; // bìa file
    }
    if (name.includes('băng') || name.includes('keo') || name.includes('dính')) {
      return '📏'; // băng keo
    }
    if (name.includes('bảng') || name.includes('tên') || name.includes('thẻ')) {
      return '🏷️'; // bảng tên
    }
    if (name.includes('chi') || name.includes('gọt') || name.includes('dao')) {
      return '✂️'; // chi gọt
    }
    if (name.includes('hóa') || name.includes('đơn') || name.includes('bill')) {
      return '🧾'; // hóa đơn
    }
    if (name.includes('máy') || name.includes('tính') || name.includes('calculator')) {
      return '🖥️'; // máy tính
    }
    if (name.includes('dụng cụ') || name.includes('phụ kiện') || name.includes('khác')) {
      return '🛠️'; // dụng cụ khác
    }
    
    // Default icon dựa trên chữ cái đầu
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-4">
              {categories.filter((category) => {
                // Chỉ hiển thị categories có ít nhất 1 sản phẩm
