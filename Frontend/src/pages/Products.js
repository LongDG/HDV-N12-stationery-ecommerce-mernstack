...
const Products = () => {
  ...
  useEffect(() => {
    fetchCategories();
    fetchAllProducts(); // Lấy tất cả sản phẩm cho việc đếm
  }, []);

  useEffect(() => {
    fetchProducts();
    
    // Set current category khi URL thay đổi
  ...
  const fetchAllProducts = async () => {
    try {
      const response = await productService.getProducts({ limit: 1000 }); // Lấy tất cả
      if (response.success) {
  ...
  const fetchProducts = async () => {
    try {
      ...
      const params = {
        ...
        // Bỏ limit cố định, để backend trả về kết quả thực tế
      };
      ...
      if (response.success) {
        ...
        // Sử dụng số lượng sản phẩm thực tế đã được filter
    ...
    <div>
      ...
      <section className="bg-gradient-to-r from-red-600 via-red-500 to-orange-500 py-8">
        <div className="swiss-container">
          ...
          <p className="text-red-100">{products.length} sản phẩm</p>
        </div>
      </section>
              ...
              <div className="mb-6">
                ...
                <div className="space-y-2">
                  <label className="flex items-center cursor-pointer group p-2 rounded hover:bg-gray-50">
                    <input 
                      ...
                      onChange={(e) => { 
                        ...
                        // Tự động apply filter khi chọn "Tất cả"
                        const params = new URLSearchParams();
                        if (searchQuery) params.set('search', searchQuery);
                        if (sortBy) params.set('sort', sortBy);
                        setSearchParams(params);
                  ...
                  {categories.filter(cat => {
                    ...
                    // Tính số lượng sản phẩm cho mỗi category từ allProducts
                    const categoryProductCount = allProducts.filter(p => {
                      const productCategoryId = p.category_id?._id || p.category_id;
                      return productCategoryId === cat._id;
                    }).length;
                    ...
                    return (
                      <label key={cat._id} className="flex items-center cursor-pointer group p-2 rounded hover:bg-gray-50">
                        <input 
                          ...
                          onChange={(e) => { 
                            setSelectedCategory(e.target.value); 
                            // Tự động apply filter khi chọn category
                            const params = new URLSearchParams();
                            if (e.target.value) {
              ...
              <div className="mb-6">
                ...
                <select 
                  ...
                  onChange={(e) => {
                    ...
                    // Tự động apply filter khi thay đổi sort
                    const params = new URLSearchParams();
                    if (searchQuery) params.set('search', searchQuery);
                    if (selectedCategory) params.set('category', selectedCategory);
